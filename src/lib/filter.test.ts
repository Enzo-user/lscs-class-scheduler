...
const catalogue = [
  makeCourse({
    ...
    sections: [
      makeSection({ id: 'CCPROG3-S11', instructor: 'Maria Santos', schedule: [slot('Monday', '09:15', '10:45'), slot('Thursday', '09:15', '10:45')] }),
      makeSection({ id: 'CCPROG3-S12', instructor: 'Jose Reyes', schedule: [slot('Tuesday', '11:00', '12:30'), slot('Friday', '11:00', '12:30')] }),
    ],
  }),
  makeCourse({
    id: 'CCDSTRU',
    title: 'Discrete Structures',
    sections: [makeSection({ id: 'CCDSTRU-S11', instructor: 'Ana Santos', schedule: [slot('Wednesday', '07:30', '09:00')] })],
  }),
  makeCourse({
    id: 'LBYARCH',
    title: 'Architecture Laboratory',
    units: 1,
    sections: [makeSection({ id: 'LBYARCH-S11', instructor: 'Paolo Cruz', schedule: [slot('Saturday', '11:00', '14:00')] })],
  }),
]
...
describe('filterCourses', () => {
  it('returns everything when no filter is set', () => {
    expect(ids(filterCourses(index, EMPTY_FILTERS))).toEqual(['CCPROG3:S11,S12', 'CCDSTRU:S11', 'LBYARCH:S11'])
  })

  it('matches code, title, instructor and section code, case-insensitively', () => {
    expect(ids(filterCourses(index, { ...EMPTY_FILTERS, query: 'ccprog' }))).toEqual(['CCPROG3:S11,S12'])
    expect(ids(filterCourses(index, { ...EMPTY_FILTERS, query: 'DISCRETE' }))).toEqual(['CCDSTRU:S11'])
    expect(ids(filterCourses(index, { ...EMPTY_FILTERS, query: 'santos' }))).toEqual(['CCPROG3:S11', 'CCDSTRU:S11'])
  ...
  it('requires every token to match (AND), across course and section text', () => {
    expect(ids(filterCourses(index, { ...EMPTY_FILTERS, query: 'prog santos' }))).toEqual(['CCPROG3:S11'])
  ...
  it('keeps sections meeting on any of the chosen days', () => {
    expect(ids(filterCourses(index, { ...EMPTY_FILTERS, days: ['Monday'] }))).toEqual(['CCPROG3:S11'])
    expect(ids(filterCourses(index, { ...EMPTY_FILTERS, days: ['Friday', 'Saturday'] }))).toEqual(['CCPROG3:S12', 'LBYARCH:S11'])
  })
