...
describe('scheduleReducer', () => {
  it('adds a section under its course', () => {
    const state = scheduleReducer(initialScheduleState, {
      type: 'ADD_SECTION',
      courseId: 'CCPROG3',
      sectionId: 'CCPROG3-S11',
    })
    expect(state.selected).toEqual({ CCPROG3: 'CCPROG3-S11' })
  })

  it('replaces the section when another one of the same course is added', () => {
    const first = scheduleReducer(initialScheduleState, {
      type: 'ADD_SECTION',
      courseId: 'CCPROG3',
      sectionId: 'CCPROG3-S11',
    })
    const second = scheduleReducer(first, {
      type: 'ADD_SECTION',
      courseId: 'CCPROG3',
      sectionId: 'CCPROG3-S12',
    })
    expect(second.selected).toEqual({ CCPROG3: 'CCPROG3-S12' })
  })

  it('keeps other courses when adding or removing', () => {
    let state = scheduleReducer(initialScheduleState, {
      type: 'ADD_SECTION',
      courseId: 'A',
      sectionId: 'A-S11',
    })
  ...
  it('returns the same state for no-op actions', () => {
    ...
    expect(scheduleReducer(state, { type: 'ADD_SECTION', courseId: 'A', sectionId: 'A-S11' })).toBe(
      state,
    )
